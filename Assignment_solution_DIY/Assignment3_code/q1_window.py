...
class Config:
    ...
    ### YOUR CODE HERE
    """注意这里也要实现
    每个windown共2*window_size+1个word，每个word的特征长度为n_word_features，由此可进行计算
    """
...
def make_windowed_data(data, start, end, window_size = 1):
    ...
    ### YOUR CODE HERE (5-20 lines)
        for i in range(len(sentence)):
            ...
            if i-window_size<0:
                add_n = window_size-i
            word_window.extend(start*add_n)
            left_idx = max(0,i-window_size)
            add_n = 0
            if i+window_size+1>len(sentence):
                add_n = i+window_size+1-len(sentence)
            right_idx = min(i+window_size+1,len(sentence))
            word_window.extend([feature for word in sentence[left_idx:right_idx] for feature in word])
            word_window.extend(end*add_n)
            windowed_data.append((word_window,labels[i]))
    ### END YOUR CODE
...
class WindowModel(NERModel):
    ...
    def add_prediction_op(self):
        ...
        with tf.variable_scope('window_ner'):
